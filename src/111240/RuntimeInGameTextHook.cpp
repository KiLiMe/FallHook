...
// Depends on RuntimeInGameTextDictionary/Log/ScaleformStream and 1.11.240 prologue hooks.
// Runtime scope is Fallout 4 1.11.240 AddTranslations/AddTranslation and literal-only BSScaleformTranslator::Translate.
// Version-specific logic: fixed 1.11.240 translator hooks and UI offsets for TerminalMenu bypass.
...
#include "111240/RuntimeInGameTextSafeCopy.h"
#include "111240/RuntimeInGameTextScaleformStream.h"
#include "111240/RuntimeInGameTextTranslateCache.h"
#include "111240/RuntimeInGameTextKeyAlias.h"
...
namespace
{
	...
	void resolveSetResultHelper()
	{
		...
		{
			REX::ERROR("{} skipped TranslateInfo::SetResult helper; unsupported 111240 bytes at {:X}.", Plugin::NAME, target.address());
			return;
		}
		g_setResult = reinterpret_cast<SetResultFunc*>(target.address());
		REX::INFO("{} resolved TranslateInfo::SetResult helper at {:X} using 111240 offset {:X}.", Plugin::NAME, target.address(), kSetResultOffset.offset());
	}
