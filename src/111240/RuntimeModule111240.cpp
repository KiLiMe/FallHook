// AI CONTEXT: Owns Fallout 4 1.11.240 hook installation and message-driven runtime callbacks.
// Depends on all active 111240 hook modules plus shared settings, logging, and watchdog helpers.
// Runtime assumptions: selected only after F4SE reports Fallout 4 runtime 1.11.240.
// Version-specific logic: yes; all hook installs here target 1.11.240-compatible modules.
...
#include "111240/RuntimeQuestJournalTextHook.h"
#include "111240/RuntimeTextManager.h"
...
namespace
{
	...
	bool Load(const F4SE::LoadInterface*)
	{
		...
		REX::INFO("{} QUST:CNAM journal hook is enabled for 1.11.240 with TESQuestStageItem::GetLogEntry.", Plugin::NAME);
	...
	const RuntimeVersionDispatcher::Module kModule{
		.name = "111240",
		.runtime = RuntimeVersionDispatcher::kRuntime111240,
		.Load = Load,
