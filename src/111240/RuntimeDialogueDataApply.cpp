// AI CONTEXT: Mutates loaded dialogue topic-name data from source-free XML records.
// Depends on RuntimeDialogueForms, RuntimeFormResolver, and CommonLibF4 dialogue forms.
// Runtime scope is Fallout 4 1.11.240 DIAL:FULL topic-name mutation only.
// Version-specific logic: none.
// Source-free policy: applies by loaded DIAL identity; INFO:NAM1 is deferred to DialogueResponse construction.
#include "PCH.h"

#include "RuntimeApplySettings.h"
#include "111240/RuntimeDialogueDataApply.h"
...
namespace
{
		...
		std::uint32_t ordinal = 0xFFFFFFFFu)
	{
		...
		REX::INFO(
			"{} dialogue-data-apply trace stage={} rec={} form={:08X} editor={} sid={:08X} index={} ordinal={} textLen={}",
			Plugin::NAME,
			stage,
	...
	[[nodiscard]] bool isDeferredResponseRecord(const TranslationCatalogRecord& record)
	{
	...
	[[nodiscard]] DialogueMaps buildMaps(const TranslationCatalogBuildResult& catalog)
	{
		...
		{
			if (isDeferredResponseRecord(record))
			{
				++maps.stats.responsesDeferred;
				continue;
			}
	...
	void countMissingTopicTargets(DialogueMaps& maps, RuntimeDialogueDataApply::ApplyStats& stats)
	{
...
namespace RuntimeDialogueDataApply
{
	ApplyStats Apply(const TranslationCatalogBuildResult& catalog)
	{
		...
		countMissingTopicTargets(maps, stats);

		REX::INFO(
			"{} dialogue data apply complete: responsesDeferred={} topics={} missingForm={} emptyText={} topicMaps={} scannedForms={} topicForms={} elapsedMs={:.2f}.",
			Plugin::NAME, stats.responsesDeferred, stats.topicsApplied, stats.skippedMissingForm,
			stats.skippedEmptyText,
			maps.topicsByForm.size() + maps.topicsByEditor.size(), forms.scannedForms, forms.topics.size(), elapsedMs(started));
		return stats;
	}
}
