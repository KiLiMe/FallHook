...
// Depends on activation look context, debug settings, and prologue hooks.
// Runtime scope is the user-approved Fallout 4 1.11.240 HUDRollover post-data action exception.
// Version-specific logic: installs verified 1.11.240 moved HUDRollover REL IDs matching the 1.10.163 hook family.
// Source-free policy: replacement uses form/sID/editor identity; visible text is diagnostic only.
#include "PCH.h"

#include "111240/RuntimeHudRolloverHook.h"
...
namespace
{
	...
	using ShowParameterCopyFunc = void(void*, void*);
	constexpr REL::ID kShowRolloverID{ 2221994 };
	constexpr REL::ID kShowActivateButtonID{ 2221996 };
	constexpr REL::ID kShowSecondaryButtonID{ 2221997 };
	constexpr REL::ID kShowRolloverNounID{ 2222000 };
	constexpr REL::ID kShowParameterCopyID{ 2222029 };
	...
	constexpr std::array<Slot, 3> kRolloverTextSlots{
		Slot{ 0x00, "+00" },
		Slot{ 0x08, "+08" },
		Slot{ 0x10, "+10" }
	};
	...
	ShowParameterCopyFunc* g_showParameterCopy{ nullptr };
	...
	RuntimeHookWatch::Stats g_parameterCopyWatch;
	...
	void traceRolloverParameters(std::string_view hook, void* rollover, void* parameters, const void* caller)
	{
		...
		if (g_traceLines.fetch_add(1, std::memory_order_relaxed) < kTraceLineLimit)
		{
			REX::INFO("{} hud-rollover trace hook={} stage=entry rollover={} params={} caller=\"{}\" activation=\"{}\"",
				Plugin::NAME, hook, pointerText(rollover), pointerText(parameters),
				escape(RuntimeInGameTextLog::FormatAddress(caller)), escape(RuntimeActivationLookContext::FormatRecent(1000)));
		}
		if (!RuntimeMemorySafety::IsReadableMemory(parameters, 0x18)) { return; }
	...
	void showParameterCopyThunk(void* destination, void* source)
	{
		RuntimeHookWatch::ScopedCall watch{ g_parameterCopyWatch, "HUDRollover::ShowParameterCopy" };
		traceRolloverParameters("HUDRollover::ShowParameterCopySource", nullptr, source, _ReturnAddress());
		g_showParameterCopy(destination, source);
		traceRolloverParameters("HUDRollover::ShowParameterCopyDestination", nullptr, destination, _ReturnAddress());
	}
...
namespace RuntimeHudRolloverHook
{
	void Install()
	{
		...
		installID(kShowRolloverID, showRolloverThunk, g_showRollover, "HUDRollover::ShowRollover");
		installID(kShowActivateButtonID, showActivateButtonThunk, g_showActivateButton, "HUDRollover::ShowActivateButton");
		installID(kShowSecondaryButtonID, showSecondaryButtonThunk, g_showSecondaryButton, "HUDRollover::ShowSecondaryButton");
		installID(kShowRolloverNounID, showRolloverNounThunk, g_showRolloverNoun, "HUDRollover::ShowRolloverNoun");
		installID(kShowParameterCopyID, showParameterCopyThunk, g_showParameterCopy, "HUDRollover::ShowParameterCopy");
	}
}
