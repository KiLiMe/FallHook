...
// Version-specific logic: installs verified 1.11.191 Address Library prologue hooks and AddTranslation stub.
...
namespace RuntimeInGameTextHookPatch
{
	...
	bool InstallAddRcx8LeaR9JumpStub(std::uintptr_t target, std::uintptr_t detour, std::uintptr_t& original)
	{
		...
		{
			REX::ERROR(
				"{} skipped BSTranslator::AddTranslation hook; unsupported 111191 stub bytes: {}",
				Plugin::NAME,
		...
		REX::INFO("{} installed BSTranslator::AddTranslation 111191 jump-stub hook at {:X}.", Plugin::NAME, target);
		return true;
	}
}
