// AI CONTEXT: Implements 1.10.163 XDI option-to-INFO resolution.
...
// Version-specific logic: uses 1.10.163 active XDI dialogue-map topic ordering only.
...
namespace
{
	constexpr REL::ID kPlayerCharacterSingletonID{ 303410 };
	...
	{
		static REL::Relocation<RE::NiPointer<RE::PlayerCharacter>*> singleton{ kPlayerCharacterSingletonID };
		auto* player = singleton->get();
...
namespace RuntimeXdiDialogueInfoResolver
{
	InfoList CollectActiveInfos()
	{
		...
		infos.reserve(16);
		for (auto* topic : state.action->responseTopics)
		{
