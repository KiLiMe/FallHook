...
// Runtime scope is Fallout 4 1.10.163 text mutation plus source-free lookup data readiness.
...
#include "110163/RuntimeApplyTrace.h"
#include "110163/RuntimeConstApplyCache.h"
#include "110163/RuntimeDescriptionTranslations.h"
...
#include "110163/RuntimePipboyLogTranslations.h"
...
namespace
{
	...
	{
		{
			RuntimeLoadWatchdog::ScopedPhase phase{ "RuntimeTextManager apply dialogue topic data", 0.0 };
			const auto started = std::chrono::steady_clock::now();
			(void)RuntimeDialogueDataApply::Apply(prepared.dialogue);
			logPhase("apply dialogue topic data", started);
		}
		{
	...
	{
		{
			RuntimeLoadWatchdog::ScopedPhase phase{ "RuntimeTextManager reapply dialogue topic data after save-load", 0.0 };
			const auto started = std::chrono::steady_clock::now();
			(void)RuntimeDialogueDataApply::Apply(prepared.dialogue);
			logPhase("reapply dialogue topic data after save-load", started);
		}
		{
...
namespace RuntimeTextManager
{
	...
	void RefreshAfterSaveLoad()
	{
		...
		if (sameCatalog)
		{
			...
			REX::INFO(
				"{} save-load refresh complete: sameCatalog=true constMapsReused={} constMapsBuilt={} dialogueTopicsReapplied=true inventoryTemplatesReapplied=true innrReapplied=true cacheHit={}.",
				Plugin::NAME,
		...
		REX::INFO(
			"{} save-load refresh complete: forceRebuiltMaps=true dialogueTopicsReapplied=true reappliedMutableData=true reappliedConstData=true bootApplied={} cacheHit={}.",
			Plugin::NAME,
			g_bootApplyComplete,
			catalogResult->loadedFromCache);
