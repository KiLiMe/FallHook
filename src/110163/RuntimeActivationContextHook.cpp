// AI CONTEXT: Captures the current activation reference/base identity for HUDRollover action replacement.
// Depends on RuntimeActivationLookContext, RuntimeApplySettings, and prologue hook support.
// Runtime scope is Fallout 4 1.10.163 TESObjectREFR::GetActivateText.
// Version-specific logic: fixed Address Library ID 1077221 for Fallout 4 1.10.163.
// Source-free policy: stores form identity for approved HUD action replacement; live text is diagnostic only.
#include "PCH.h"
...
namespace
{
	...
	constexpr REL::ID kGetActivateTextID{ 1077221 };
		...
		bool result)
	{
		...
		RuntimeActivationLookContext::Update(
			...
			traceEnabled ? editorID(reference) : "",
			formID(base),
			formLocalID(base),
			baseTypeID,
			traceEnabled ? formTypeString(baseTypeID) : "",
			traceEnabled ? editorID(base) : "",
			result,
		...
		REX::INFO(
			"{} activation-context trace ref={:08X} refType={} refEditor={} base={:08X} baseLocal={:06X} baseType={} baseEditor={} result={} textLen={}",
			Plugin::NAME,
			formID(reference),
